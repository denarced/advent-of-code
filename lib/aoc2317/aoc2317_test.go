...
func TestSumHasher(t *testing.T) {
	...
	hasher := newSumHasher(0, 0)

	first, _, _ := hasher.hash(
		shared.Loc{X: 0, Y: 33},
		shared.RealEast,
		0)
	second, _, _ := hasher.hash(
		shared.Loc{X: 1, Y: 0},
		shared.RealEast,
		0)
	req.NotEqual(first, second, "hash mismatch")

	values := map[uint64]string{}
	for x := range 100 {
		for y := range 100 {
			for _, dir := range []shared.Direction{
				shared.RealEast,
				shared.RealSouth,
				shared.RealWest,
				shared.RealNorth,
				...
				for arrow := range 4 {
					...
					h, _, _ := hasher.hash(loc, dir, arrow)
					s := fmt.Sprintf("%v-%v-%v", loc, dir, arrow)
					current, ok := values[h]
					req.Falsef(ok, "%d - %s - %s", h, current, s)
					values[h] = s
				}
			}
		}
